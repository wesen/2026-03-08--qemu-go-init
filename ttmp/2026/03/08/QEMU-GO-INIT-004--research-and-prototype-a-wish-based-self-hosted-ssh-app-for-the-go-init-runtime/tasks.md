...
- [x] Wire the SSH service into the PID 1 boot sequence and use a stable guest host-key path
- [x] Expose SSH state in `/api/status` and the embedded webpage
- [x] Extend the QEMU run/smoke tooling with SSH port forwarding and host-side validation
- [x] Validate the full HTTP + SSH guest workflow and refresh the ticket docs, diary, and bundle
